/* 
  ...
  Created on May 30, 2017, 12:15 PM
  Purpose: A friendly game of Battleship
 */
...
const char H20 = 247; //ASCII Character Code
const char HitShip = 'X';
const char ShipPlace = 'S';
const char MissShip = '0';
struct POINT {
...
int main()
{
	...
	for (int aplyr=1; aplyr<3; ++aplyr)
	{
		...
		for (int thisShip=0; thisShip<SHIP_TYPES; ++thisShip)
		{
			...
			cout<<"Assign the placement for your ships.";
                        cout<<"The Placement Format should be:\n";
			cout<<"0: For Horizontal Direction\n";
                        cout<<"1: For Vertical Direction\n";
                        cout<<"Top-row (X-coordinates)\n";
                        cout<<"Left-side (Y-coordinates)\n";
			cout<<"Example: 0 1 2\n";
                        cout<<"This would place a ship beginning at X:1 Y:2 going horizontal\n";
                        cout<<"*************************************************************\n";
                        cout<<"Ship Placing Phase\n";
                        cout<<"-------------------\n";
			cout<<"Current Ship being placed: " << ship[thisShip].name 
			...
			player[aplyr].grid[aShip.shipType.onGrid[0].X][aShip.shipType.onGrid[0].Y] = ShipPlace;
			...
			for (int i=1; i<aShip.shipType.length; ++i)
			{
				...
				player[aplyr].grid[aShip.shipType.onGrid[i].X][aShip.shipType.onGrid[i].Y] = ShipPlace;
			}
	...
	do {
		...
		if (player[enemyPlayer].grid[x][y] == ShipPlace) player[enemyPlayer].grid[x][y] = HitShip;
		if (player[enemyPlayer].grid[x][y] == H20) player[enemyPlayer].grid[x][y] = MissShip;
	...
	system("cls");
	cout << "\n\nYOU SUNK MY BATTLESHIP!!!  PLAYER " << thisPlayer << " HAS WON THE GAME!\n\n\n\n";
	system("pause");
	return 0;
}
bool GameOverCheck(int enemyPLAYER)
{
	...
	for (int w=0; w<BOARD_WIDTH; ++w){
			for (int h=0; h<BOARD_HEIGHT; ++h){
				...
				if (player[enemyPLAYER].grid[w][h] = ShipPlace)
					{
						winner = false;
						return winner;
					}
...
void DrawBoard(int thisPlayer)
{
	...
	for (int h=0; h<BOARD_HEIGHT; ++h){
		for (int w=0; w<BOARD_WIDTH; ++w){			
			...
			//Do not show ships, show damage if it's hit instead
			if (gameRunning == true && player[thisPlayer].grid[w][h] != ShipPlace) 
			{cout << player[thisPlayer].grid[w][h] << "  ";}
			else if (gameRunning == true && player[thisPlayer].grid[w][h] == ShipPlace)
			{cout << H20 << "  ";}	
